...
def update_popular_stats(stream_type):
    ...
    df_top_discover = df_popular.groupby(
        ["publisher_id"], sort=False
    ).agg(view_count =  ('view_count', 'sum'), discover_count = ('discovered_at', 'count')).reset_index()

    df_top = df_top_discover.sort_values(by=["discover_count", "view_count"], ascending=False)
    df_top_channels_discover = df_top.head(5)[["publisher_id"]]

    df_top_channels = df_top_channels_discover.merge(
